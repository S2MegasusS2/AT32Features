...
lcd_dev_type lcd_dev_struct =
{
  ...
  lcd_write_string, /*!< lcd function to write char */
  lcd_write_num /*!< lcd function to write number */
};
...
/**
  * @brief  this function is write one character.
  * @param  character : the character to write.
  * @retval none
  */
void lcd_write_char(char character, uint16_t row_start, uint16_t col_start, uint16_t color)
{

	//	printf("row: %d\r\n", row_bit_index + index_char_Row);
	//	printf("col: %d\r\n", col_bit_index + index_char_Col);
	uint8_t row_bit_index = row_start * 8;
	uint8_t col_bit_index = col_start * 8;
	const uint8_t* characterFont = font8x8_basic_tr[(uint8_t)character];

	uint32_t index_char_Col = 0;
	uint32_t index_char_Row = 0;

	uint8_t bit;

	lcd_set_block(row_bit_index, col_bit_index, row_bit_index + 7, col_bit_index + 7);

	for (index_char_Row = 0; index_char_Row < 8; index_char_Row++)
	{
		for (index_char_Col = 0; index_char_Col < 8; index_char_Col++)
		{
			bit = (characterFont[index_char_Row] >> (index_char_Col + 1)) & 0x01;
			if(bit == 0x01)
			{
				lcd_draw_point(row_bit_index + index_char_Row, col_bit_index + index_char_Col, RED);
			}
			else
			{
				lcd_draw_point(row_bit_index + index_char_Row, col_bit_index + index_char_Col, GREEN);
			}
		}
	}
}
...
/**
  * @brief  this function is write one string.
  * @param  string : the string to write.
  * @param  row_start : start of row between 0 and 39
  * @param  col_start : start of column between 0 and 29
  * @param  color : the color of string showed on lcd
  * @retval none
  */
void lcd_write_string(char *string, uint16_t row_start, uint16_t col_start, uint16_t color)
{
	uint16_t string_index = 0;

	uint16_t true_row = col_start;
	uint16_t true_col = row_start;

	for (string_index = 0; string[string_index] != '\0'; string_index++){
		lcd_write_char(string[string_index], true_row++, true_col, color);
	}
}
...
void lcd_write_num(uint16_t num, uint16_t row_start, uint16_t col_start, uint16_t color)
{
	...
	{
		lcd_write_char(number_string[i], col_start++, row_start, color);
	}
}
